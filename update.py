...
def check_sinfo_update(db, update_date):
...
def update_sinfo_and_schema(db, update_date):
    ...
    logger = qutils.Log().get_logger('file')
    ...
    with tqdm(total=100, ascii=True, desc='Sinfo Table UPDATE') as pbar:
        ...
        for wics_code in configUpdate.WICS_MC.keys():
            ...
            if response.status_code == 200:  #
                ...
                for json in json_list['list']:
                    ...
                    market, num_stocks = crowling_market_and_numstocks(code)
        ...
        for code in only_new_list:
            ...
            listing_date = crowling_listing_date(code=code)
...
def crowling_market_and_numstocks(code):
    logger = qutils.Log().get_logger('file')
...
def crowling_listing_date(code):
    logger = qutils.Log().get_logger('file')
